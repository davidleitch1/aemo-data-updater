#!/usr/bin/env python3
"""
Simple merge script to add backfill data to existing parquet
        ...
        try:
            ...
            if start <= file_time <= end:
                ...
                with zipfile.ZipFile(io.BytesIO(nested_content)) as inner_zip:
                    ...
                    # Parse
                    df = collector.parse_mms_csv(csv_content, 'PRICE')

                    if not df.empty and 'SETTLEMENTDATE' in df.columns:
                        price_df = pd.DataFrame()
                        price_df['settlementdate'] = pd.to_datetime(df['SETTLEMENTDATE'].str.strip('"'), format='%Y/%m/%d %H:%M:%S')
                        price_df['regionid'] = df['REGIONID'].str.strip()
                        price_df['rrp'] = pd.to_numeric(df['RRP'], errors='coerce')
                        price_df = price_df[price_df['regionid'].isin(['NSW1', 'QLD1', 'SA1', 'TAS1', 'VIC1'])]

                        if not price_df.empty:
                            all_price_data.append(price_df)
                            print(f"  {file_time}: {len(price_df)} records")
        except:
            pass

print(f"\nCollected {len(all_price_data)} files")

# Combine
backfill_df = pd.concat(all_price_data, ignore_index=True)
backfill_df = backfill_df.drop_duplicates(subset=['settlementdate', 'regionid'])
backfill_df = backfill_df.sort_values(['settlementdate', 'regionid'])
...
# Load existing
existing_df = pd.read_parquet('/Volumes/davidleitch/aemo_production/data/prices5.parquet')
...
# Save
print("\nSaving...")
combined.to_parquet('/Volumes/davidleitch/aemo_production/data/prices5.parquet', compression='snappy', index=False)
